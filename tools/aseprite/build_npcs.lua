-- Render the NPC design library (humans this pass) as 2-frame idle strips.
-- Approval/QC only (_-prefixed, not deployed).
local DIR = "C:/Users/rober/Documents/aat_app/tools/aseprite/"
local M = dofile(DIR.."chars.lua")
local L = dofile(DIR.."lib.lua")
local OUT = DIR.."out/"
local function saveStrip(spec, slug)
  local f0=M.human(spec,0); local f1=M.human(spec,1)
  local s=L.strip({f0,f1}); s:save(OUT.."_npc-"..slug..".png"); s:close(); f0:close(); f1:close()
end

local S,H,SH,C,T = M.SKIN, M.HAIR, M.SHIRT, M.CLOTH, M.TIE
local HUMANS = {
...
for _,e in ipairs(HUMANS) do saveStrip(e[2], e[1]) end
print("NPCS_HUMAN_OK "..#HUMANS)

local function saveAnimal(spec, slug)
  local f0=M.animal(spec,0); local f1=M.animal(spec,1)
  local s=L.strip({f0,f1}); s:save(OUT.."_npc-"..slug..".png"); s:close(); f0:close(); f1:close()
end
local F = M.FUR
local ANIMALS = {
...
for _,e in ipairs(ANIMALS) do saveAnimal(e[2], e[1]) end
print("NPCS_ANIMAL_OK "..#ANIMALS)
